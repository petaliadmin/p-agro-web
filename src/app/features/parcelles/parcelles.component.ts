...
import { BilanHydrique } from '../../core/models/irrigation.model';
import { NdviData, getNdviColor, getNdviClasse, getNdviClasseLabel } from '../../core/models/ndvi.model';
  ...
  template: `
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-2 sm:gap-4 mb-6">
      <div class="card p-4 text-center">
        <p class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ parcelles.length }}</p>
        <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Parcelles totales</p>
      </div>
      <div class="card p-4 text-center">
        <p class="text-2xl font-bold text-primary-600 dark:text-primary-400">{{ totalHa }}</p>
        <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Hectares couverts</p>
      </div>
      <div class="card p-4 text-center">
        <p class="text-2xl font-bold text-red-600 dark:text-red-400">{{ urgentesCount }}</p>
        <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Parcelles urgentes</p>
      </div>
      <div class="card p-4 text-center">
        <p class="text-2xl font-bold text-yellow-600 dark:text-yellow-400">{{ attentionCount }}</p>
        <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">À surveiller</p>
      </div>
    </div>
      ...
      <div class="flex flex-wrap items-center gap-3">
        ...
        <div class="relative flex-1 min-w-48">
          ...
          <input type="text" [(ngModel)]="search" (ngModelChange)="applyFilters()"
            ...
            class="w-full pl-8 pr-3 py-2 text-sm border border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-400"/>
        </div>
        <!-- Zone -->
        <select [(ngModel)]="filterZone" (ngModelChange)="applyFilters()" aria-label="Filtrer par zone"
          class="text-sm border border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-400 text-gray-700">
        ...
        <!-- Source eau -->
        <select [(ngModel)]="filterSourceEau" (ngModelChange)="applyFilters()" aria-label="Filtrer par source d'eau"
          class="text-sm border border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-400 text-gray-700">
          <option value="">Toutes sources d'eau</option>
        ...
        <!-- Campagne -->
        <select [(ngModel)]="filterCampagne" (ngModelChange)="applyFilters()" aria-label="Filtrer par campagne"
          class="text-sm border border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-400 text-gray-700">
        ...
        <!-- NDVI -->
        <select [(ngModel)]="filterNdvi" (ngModelChange)="applyFilters()" aria-label="Filtrer par classe NDVI"
          class="text-sm border border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-400 text-gray-700">
          <option value="">NDVI : tous</option>
          <option value="sain">Sain (> 0.6)</option>
          <option value="attention">Attention (0.3–0.6)</option>
          <option value="stress">Stress (&lt; 0.3)</option>
        </select>
        <!-- Export -->
        <div class="relative ml-auto">
          ...
          <div *ngIf="showExportMenu" class="absolute right-0 mt-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-lg shadow-lg z-20 py-1 min-w-[180px] animate-dropdown">
            <button (click)="exportCSV(false); showExportMenu = false" class="w-full text-left px-3 py-2 text-xs text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
              <span class="material-icons text-[14px] mr-1 align-middle" aria-hidden="true">filter_list</span> Exporter filtrés ({{ filtered.length }})
            </button>
            <button (click)="exportCSV(true); showExportMenu = false" class="w-full text-left px-3 py-2 text-xs text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
    ...
    <div *ngIf="vue === 'liste'" class="card overflow-hidden">
          ...
          <tr *ngFor="let p of paginatedParcelles; trackBy: trackById"
            ...
            <td class="table-cell">
              <div class="flex items-center gap-1.5">
                ...
                <span *ngIf="bilansHydriques[p.id]?.stressHydrique" class="inline-flex items-center"
                  [title]="'Stress hydrique : ' + bilansHydriques[p.id].niveauStress">
                  <span class="material-icons text-[14px]"
                    [class.text-yellow-500]="bilansHydriques[p.id].niveauStress === 'leger'"
                    [class.text-orange-500]="bilansHydriques[p.id].niveauStress === 'modere'"
                    [class.text-red-500]="bilansHydriques[p.id].niveauStress === 'severe'" aria-hidden="true">water_drop</span>
                </span>
              </div>
            </td>
            <td class="table-cell text-xs">{{ getTechNom(p.technicienId) }}</td>
            <td class="table-cell">
              <span *ngIf="ndviByParcelle[p.id]" class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-semibold"
                [style.background]="getNdviColor(ndviByParcelle[p.id].ndviMoyen) + '20'"
                [style.color]="getNdviColor(ndviByParcelle[p.id].ndviMoyen)">
                <span class="w-1.5 h-1.5 rounded-full" [style.background]="getNdviColor(ndviByParcelle[p.id].ndviMoyen)"></span>
                {{ ndviByParcelle[p.id].ndviMoyen | number:'1.2-2' }}
              </span>
              <span *ngIf="!ndviByParcelle[p.id]" class="text-xs text-gray-400">—</span>
            </td>
      ...
      <!-- Pagination -->
      <div *ngIf="!loading && filtered.length > 0" class="flex flex-wrap items-center justify-between gap-3 px-5 py-3 border-t border-gray-100 dark:border-gray-700">
        <div class="flex items-center gap-3">
          <span class="text-xs text-gray-500 dark:text-gray-400">Affichage {{ (page - 1) * pageSize + 1 }}-{{ page * pageSize > filtered.length ? filtered.length : page * pageSize }} sur {{ filtered.length }} parcelles</span>
          <select [(ngModel)]="pageSize" (ngModelChange)="onPageSizeChange()" class="text-xs border border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 rounded px-2 py-1.5 min-h-[36px]" aria-label="Éléments par page">
        ...
        <div class="flex items-center gap-1">
          <button (click)="goToPage(page - 1)" [disabled]="page <= 1" class="px-3 py-2 min-h-[44px] text-xs rounded border border-gray-200 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 disabled:opacity-40 disabled:cursor-not-allowed dark:text-gray-300">Préc.</button>
          <ng-container *ngFor="let pg of getVisiblePages()">
            <span *ngIf="pg === -1" class="px-1 text-gray-400">…</span>
            <button *ngIf="pg !== -1" (click)="goToPage(pg)"
              class="px-3 py-2 min-h-[44px] min-w-[44px] text-xs rounded border transition-colors"
              [ngClass]="page === pg
                ? 'bg-primary-600 text-white border-primary-600'
                : 'border-gray-200 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 dark:text-gray-300'">{{ pg }}</button>
          </ng-container>
          <button (click)="goToPage(page + 1)" [disabled]="page >= totalPages" class="px-3 py-2 min-h-[44px] text-xs rounded border border-gray-200 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 disabled:opacity-40 disabled:cursor-not-allowed dark:text-gray-300">Suiv.</button>
          <div *ngIf="totalPages > 5" class="flex items-center gap-1 ml-2">
            <input type="number" [min]="1" [max]="totalPages" [(ngModel)]="goToPageInput" placeholder="Page"
              class="w-16 text-xs border border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 rounded px-2 py-1.5 min-h-[36px]" aria-label="Aller à la page" />
            <button (click)="goToPage(goToPageInput)" class="px-2 py-1.5 min-h-[36px] text-xs rounded border border-gray-200 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 dark:text-gray-300">Go</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div *ngIf="vue === 'grille'" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
      <div *ngFor="let p of filtered; trackBy: trackById"
        class="card p-5 cursor-pointer hover:shadow-md dark:hover:shadow-gray-900/50 transition-shadow" [routerLink]="['/parcelles', p.id]">
        <div class="flex items-start justify-between mb-3">
          <div>
            <p class="font-semibold text-gray-900 dark:text-gray-100">{{ p.nom }}</p>
            <p class="text-xs text-gray-500 dark:text-gray-400">{{ p.code }}</p>
          </div>
          <div class="flex items-center gap-1.5">
            <app-status-chip [statut]="p.statut"></app-status-chip>
            <span *ngIf="bilansHydriques[p.id]?.stressHydrique" [title]="'Stress hydrique : ' + bilansHydriques[p.id].niveauStress">
              <span class="material-icons text-[14px]"
                [class.text-yellow-500]="bilansHydriques[p.id].niveauStress === 'leger'"
                [class.text-orange-500]="bilansHydriques[p.id].niveauStress === 'modere'"
                [class.text-red-500]="bilansHydriques[p.id].niveauStress === 'severe'" aria-hidden="true">water_drop</span>
        ...
        <div class="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400 border-t border-gray-100 dark:border-gray-700 pt-3">
          <span>{{ p.zone }}</span>
          <span *ngIf="ndviByParcelle[p.id]" class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded font-semibold"
            [style.background]="getNdviColor(ndviByParcelle[p.id].ndviMoyen) + '20'"
            [style.color]="getNdviColor(ndviByParcelle[p.id].ndviMoyen)">
            <span class="material-icons text-[10px]" aria-hidden="true">satellite_alt</span>
            {{ ndviByParcelle[p.id].ndviMoyen | number:'1.2-2' }}
          </span>
          <span *ngIf="!ndviByParcelle[p.id]">{{ p.derniereVisite | date:'dd/MM/yy' }}</span>
        </div>
      </div>
    </div>
    ...
    <div *ngIf="vue === 'carte'" class="card overflow-hidden" style="height: 600px; position: relative;">
      ...
      <!-- Surface panel -->
      <div *ngIf="drawnLayers.length" class="absolute top-3 right-3 z-[1000] bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-600 p-4 min-w-[240px]">
        ...
        <div class="space-y-2">
          <div *ngFor="let layer of drawnLayers; let i = index" class="flex items-center justify-between bg-gray-50 dark:bg-gray-700 rounded-lg px-3 py-2">
        ...
        <div class="border-t border-gray-200 dark:border-gray-600 mt-3 pt-3 flex items-center justify-between">
          <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Total</span>
          <span class="text-base font-bold text-primary-600 dark:text-primary-400">{{ totalDrawnArea }}</span>
        </div>
...
export class ParcellesComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  bilansHydriques: Record<string, BilanHydrique> = {};
  ndviByParcelle: Record<string, NdviData> = {};
  filterNdvi = '';

  private pendingDrawData: any = null;
  ...
  constructor(
    ...
    // Capture Router state before it is cleared
    const nav = this.router.getCurrentNavigation();
    this.pendingDrawData = nav?.extras?.state?.['drawData'] || null;
  }
  ...
  ngOnInit(): void {
    this.parcelleService.getAll().subscribe(data => {
      ...
      // Load bilans hydriques for stress indicators
      data.forEach(p => {
        this.irrigationService.getBilanHydrique(p.id).pipe(take(1)).subscribe(bh => {
          this.bilansHydriques[p.id] = bh;
          this.cdr.markForCheck();
        });
      });
      // Load NDVI data
      ...
      // Check for draw data from carte publique
      const drawData = this.pendingDrawData || this.getSessionDrawData();
      if (drawData) {
        setTimeout(() => this.openCreateFromDraw(drawData), 500);
      }
    });
  }

  private getSessionDrawData(): any {
    try {
      const raw = sessionStorage.getItem('agroassist_draw_data');
      if (raw) {
        sessionStorage.removeItem('agroassist_draw_data');
  ...
  private async openCreateFromDraw(drawData: any): Promise<void> {
    const { ParcelleFormComponent } = await import('./parcelle-form.component');
    const ref = this.dialogService.open(ParcelleFormComponent, { data: { prefilledDraw: drawData } });
  ...
  applyFilters(): void {
    let result = this.parcelles.filter(p => {
      ...
      const matchSourceEau = !this.filterSourceEau || p.sourceEau === this.filterSourceEau;
      const matchCampagne = !this.filterCampagne || p.typeCampagne === this.filterCampagne;
      const matchNdvi = !this.filterNdvi || (this.ndviByParcelle[p.id] && getNdviClasse(this.ndviByParcelle[p.id].ndviMoyen) === this.filterNdvi);
      return matchSearch && matchZone && matchCulture && matchStatut && matchSourceEau && matchCampagne && matchNdvi;
    });
    if (this.sortField) {
  ...
  private initMap(): void {
    ...
    const ndviPolygonsGroup = L.featureGroup();
    ...
    this.filtered.forEach(p => {
      ...
      const ndvi = this.ndviByParcelle[p.id];
      const ndviColor = ndvi ? getNdviColor(ndvi.ndviMoyen) : null;
      const ndviClasse = ndvi ? getNdviClasseLabel(getNdviClasse(ndvi.ndviMoyen)) : '—';
      const popupContent = `
        <div style="min-width:200px">
          ...
          <div style="margin:6px 0;font-size:12px">
            ...
            ${ndvi ? `<br/><span>NDVI : <span style="color:${ndviColor};font-weight:bold">${ndvi.ndviMoyen.toFixed(2)}</span> (${ndviClasse})</span>` : ''}
          </div>
      ...
      if (p.geometry && p.geometry.length >= 3) {
        ...
        // Polygone statut normal
        L.polygon(latlngs, { color, weight: 2, fillColor: color, fillOpacity: 0.2 })
          .addTo(this.mapInstance)
          .bindPopup(popupContent);

        // Polygone NDVI (overlay séparé)
        if (ndvi) {
          const ndviPoly = L.polygon(latlngs, {
            color: ndviColor!,
            weight: 2,
            fillColor: ndviColor!,
            fillOpacity: 0.5,
          }).bindPopup(popupContent)
            .bindTooltip(`<b>${p.nom}</b><br/>NDVI: <b style="color:${ndviColor}">${ndvi.ndviMoyen.toFixed(2)}</b> (${ndviClasse})`, {
              direction: 'top', offset: [0, -10],
            });
          ndviPolygonsGroup.addLayer(ndviPoly);
        }
      }
    });

    // Contrôle de couches NDVI
    const overlays: Record<string, L.LayerGroup> = { 'Couche NDVI': ndviPolygonsGroup };
    L.control.layers(undefined, overlays, { position: 'topright', collapsed: false }).addTo(this.mapInstance);
