...
@Component({
  ...
  template: `
    <div class="fixed inset-0 z-[100] flex items-center justify-center p-4" (click)="onBackdropClick($event)">
      ...
      <div class="relative bg-white rounded-2xl shadow-2xl w-full transform overflow-hidden" role="dialog" aria-modal="true" [attr.aria-label]="title" cdkTrapFocus [cdkTrapFocusAutoCapture]="true"
        [ngClass]="{
          'max-w-md': size === 'sm',
          'max-w-lg': size === 'md',
          'max-w-2xl': size === 'lg',
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div>
            ...
            <p *ngIf="subtitle" class="text-xs text-gray-500 mt-0.5">{{ subtitle }}</p>
          </div>
          <button (click)="onClose()" class="w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-colors" aria-label="Fermer la boîte de dialogue">
...
export class FormDialogComponent {
  ...
  onBackdropClick(event: MouseEvent): void {
    if (!this.loading && (event.target as HTMLElement).classList.contains('fixed')) {
      this.onClose();
    }
  }
}
